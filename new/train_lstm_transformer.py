...
class HybridLSTMTransformer(nn.Module):
    ...
    This model processes a sequence of input frames and predicts a single output
    for the frame following the sequence, similar to the LSTMPredictor model.
    The model uses both LSTM and Transformer components to capture different aspects
    of the sequential patterns, then combines their outputs for the final prediction.
    """
    def __init__(self, input_size, hidden_size, output_size, 
        ...
        # Ensure transformer_hidden is divisible by nhead
        # First calculate raw transformer hidden size
        raw_transformer_hidden = hidden_size - self.lstm_hidden
        # Adjust to make it divisible by nhead
        self.transformer_hidden = (raw_transformer_hidden // nhead) * nhead
        # If we rounded down to zero, set a minimum size
        if self.transformer_hidden == 0:
            self.transformer_hidden = nhead
        ...
        # Combine LSTM and Transformer outputs - now only for the last timestep
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output prediction for the last timestep [batch, output_size]
        """
        ...
        # Combine LSTM and Transformer outputs - only take the last timestep
        combined = torch.cat([lstm_out[:, -1, :], transformer_out[:, -1, :]], dim=-1)
        
        # Generate prediction for the last timestep only
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, patience=10):
    """Train the Hybrid LSTM-Transformer model."""
...
def objective(trial, data_loaders, input_features, output_features, device, leg_prefix):
    ...
    config = {
        ...
        'nhead': trial.suggest_categorical('nhead', [4, 8, 16]),  # Use only 4, 8, or 16 heads for better divisibility
        'dropout': trial.suggest_float('dropout', 0.1, 0.5),
        'learning_rate': trial.suggest_float('learning_rate', 1e-4, 1e-2, log=True),
        'lstm_ratio': trial.suggest_float('lstm_ratio', 0.5, 0.8, step=0.1),  # Use higher min ratio to ensure enough capacity for LSTM
    }
...
def main():
    """Main function to train and evaluate Hybrid LSTM-Transformer models for all 6 legs."""
    ...
    print("Using a Hybrid LSTM-Transformer architecture which combines benefits of both models.")
    print("Like the LSTM model, this model uses a window of 50 frames to predict the next frame.")
    ...
    for leg_name, leg_info in legs.items():
        try:
            ...
            leg_dir = Path(f"lstm_transformer_results/{leg_name}")
            ...
            best_model = HybridLSTMTransformer(
                input_size=len(input_features),
                output_size=len(output_features),
                ...
                lstm_ratio=best_params['lstm_ratio']
            )
            ...
            model_save_path = leg_models_dir / f'{leg_name}_hybrid_lstm_transformer_{timestamp}.pth'
            ...
            torch.save({
                ...
                'loss_type': 'MAE',
                'model_type': 'HybridLSTMTransformer',
            ...
            print(f"\nBest hybrid LSTM-Transformer model and results saved to: {model_save_path}")
